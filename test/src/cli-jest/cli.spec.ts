...
describe('CLI simple flags', () => {
    ...
    describe('when no tsconfig.json provided with just -p', () => {
        ...
        it('should display error message', () => {
            expect(command.stdout.toString()).toContain('Please provide a tsconfig file.');
        });
    });
    ...
    describe('when just serving without generation', () => {
        ...
        it('should display error message', () => {
            expect(command.stdout.toString()).toContain('./documentation/ folder doesn');
        });
    });
    ...
    describe("when just serving without generation and folder which does't exist", () => {
        ...
        it('should display error message', () => {
            expect(command.stdout.toString()).toContain("folder doesn't exist");
        });
    });
    ...
    describe('when no README/package.json files available', () => {
        ...
        it('should display error message', () => {
            ...
            expect(output.indexOf('Continuing without README.md file') > -1).toBeTruthy();
            expect(output.indexOf('Continuing without package.json file') > -1).toBeTruthy();
        });
    });
    ...
    describe('when specific files are included in tsconfig', () => {
        ...
        it('should only create links to files included via tsconfig', () => {
            ...
            expect(moduleFile).toContain('modules/FooModule.html');
            expect(moduleFile).toEqual(expect.not.stringContaining('components/BarComponent.html'));
            expect(moduleFile).toEqual(expect.not.stringContaining('injectables/FooService.html'));
            expect(moduleFile).toEqual(expect.not.stringContaining('modules/BarModule.html'));
        });
    });
});
